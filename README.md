local Rayfield = loadstring(game:HttpGet('https://pastebin.com/raw/C1fTAzyT'))()---我不信你要二改？
...
local Toggle = farmTab:CreateToggle({
   Name = "超级帧数不同游戏有不同的效果",
   CurrentValue = false,
   Flag = "改善帧数", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
   Callback = function(Value)
  local decalsyeeted = true -- 这可以让你的帧数提高些
local g = game
local w = g.Workspace
local l = g.Lighting
local t = w.Terrain
...
game:GetService('RunService').RenderStepped:connect(function()
	if ENABLED then
		...
		if (TARGET ~= false) then
			...
			GUI_TARGET.Text = '自瞄 : '.. TARGET.Name:sub(1, 5)
		else
			GUI_TARGET.Text = '自瞄 : OFF'
		end
	end
end)
